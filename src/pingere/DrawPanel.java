...
public class DrawPanel extends JPanel {
	...
	private class MouseMotionAdapter implements MouseMotionListener {
		...
		// Here's where the magic happens. We get the current end point of the
		// mouse
		...
		public void mouseDragged(MouseEvent event) {
			...
			// If we are using the brush we draw a line from start to finish
			// then
			// set the start of the next line to be the end of the current one.
			// This way we draw an almost infinite number of lines as the user
			// drags
			// their mouse but it comes together to be seen as one.
			if (State.getTool() == State.Tool.Brush) {
				Shape line = new Shape(startX, startY, endX, endY, State.getColor(), Shape.Type.Brush,
						State.getStroke());
				shapes.add(line);
				...
				// The same logic applies for Eraser, but the size is bigger and
				// it's white
				// to blend into the background and supersede the previous
				// shapes it covered
			} else if (State.getTool() == State.Tool.Eraser) {
				Shape erased = new Shape(startX, startY, endX, endY, State.getColor(), Shape.Type.Eraser,
						State.getStroke());
				shapes.add(erased);
				...
				// Here we find the coordinates of the redrawable shapes
				// (ellipse and rectangle)
			} else {
				// We use Math.min for startX and endX (and Y) because we want
				// to support
				// the user drawing the shape in a direction before the startX/Y
				// which
				// would mean that the place where we start drawing (the top
				// left corner
				// of the shape) is actually endX/Y not startX/Y
				x = Math.min(startX, endX);
				y = Math.min(startY, endY);

				// We use Math.abs because we want to support the user drawing
				// in any
				// direction. The height and width will always be the same
				// regardless
			...
			// We repaint at the end to ensure that the user's dragging changes
			// are updated
			repaint();
		}
	}
	...
	// get the start point of when the user started dragging and the release
	// point
